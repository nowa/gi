...
- Pi source files audited: `150`.
- Explicit Pi test cases tracked separately: `1225`, with `0` remaining `待实现`, `待审计`, or `需要协议 runtime`.
- Source-level status: **complete for the current Pi coding-agent source
  audit**. The remaining differences are intentional Gi product/protocol
  choices rather than unimplemented Pi source behavior. Gi now has a default
...
- Latest keybindings increment: default TTY app hotkeys, extension shortcut
  ...
  Startup compact/expanded help, the bottom input hint, and the scoped-model
  save header now render from the same active keybinding sources.
...
| `bun/restore-sandbox-env.ts` | `已有` | `restore_sandbox_env.go`, `restore_sandbox_env_test.go` |
...
| `utils/fs-watch.ts`, `tools-manager.ts`, `highlight-js-lib-index.d.ts` | `不适用 / Go-native`; no Node FSWatcher/downloaded JS tool manager in Gi core | `footer_data_provider.go`, `package_manager.go`, protocol package docs |
...
| `components/tool-execution.ts`, `bash-execution.ts`, `diff.ts`, `visual-truncate.ts` | `已有`; updated pending tool behavior uses in-place indexed updates when available, and tool image blocks honor `showImages` / `imageWidthCells` with inline protocol rendering or image fallback text | `tool_execution_component.go`, `bash_execution.go`, `edit_tool_definition.go` |
...
| `core/session-manager.ts`, `session-cwd.ts`, `source-info.ts`, `timings.ts` | `已有 / Go-native`; runtime startup uses selected-session cwd for resources, reports missing stored cwd values, prompts for an interactive cwd override before runtime creation, and supports `GI_TIMING`/`PI_TIMING` startup timing output | `session_manager.go`, `session_cwd.go`, `startup_timings.go`, `cli_print_mode.go`, `cli_missing_cwd.go`, session manager suites |
| `core/model-registry.ts`, `model-resolver.ts`, `provider-display-names.ts`, `auth-storage.ts`, `auth-guidance.ts`, `defaults.ts`, `settings-manager.ts`, `resolve-config-value.ts` | `已有 / Go-native` | `model_registry.go`, `model_resolver.go`, `auth_storage.go`, `auth_guidance.go`, `settings_manager.go` |
...
| `core/system-prompt.ts`, `compaction/index.ts`, `compaction/branch-summarization.ts`, `compaction/compaction.ts`, `compaction/utils.ts` | `已有`; default prompts include Gi-native documentation pointers for Gi/protocol/package/extension questions, loaded AGENTS/CLAUDE context files, custom replacement prompts, and appended prompt sections | `system_prompt.go`, `system_prompt_docs.go`, `resource_loader.go`, `gi-agent-core/harness/compaction.go`, compaction tests |
| `core/bash-executor.ts`, `exec.ts`, `output-guard.ts` | `已有 / Go-native`; explicit writers replace global stdout patching, and bash output uses a bounded accumulator plus temp-file preservation for large output | `bash_executor.go`, `bash_output_accumulator.go`, `stdout_cleanliness_test.go` |
| `core/footer-data-provider.ts`, `keybindings.ts`, `telemetry.ts` | `已有 / Go-native` | `footer_data_provider.go`, `keybindings.go`, `telemetry.go` |

### Core Tools

| Pi files | Disposition | Gi evidence |
|---|---|---|
| `tools/read.ts`, `write.ts`, `edit.ts`, `edit-diff.ts`, `file-mutation-queue.ts`, `tool-definition-wrapper.ts`, `render-utils.ts`, `truncate.ts` | `已有`; Go SDK/protocol wrappers preserve tool schemas, prepare hooks, execution metadata, and provider-facing active tool definitions instead of copying Pi's TypeScript wrapper API | `file_tools.go`, `edit_tool_definition.go`, `file_mutation_queue.go`, `sdk_session.go`, read/write/edit tests |
| `tools/bash.ts`, `grep.ts`, `find.ts`, `ls.ts`, `path-utils.ts`, `output-accumulator.ts`, `index.ts` | `已有 / Go-native`; bash live updates and final results use bounded tail snapshots plus temp-file preservation | `bash_tool.go`, `bash_output_accumulator.go`, `search_tools.go`, `file_tool_definitions.go`, tools tests |

### Export HTML

| Pi files | Disposition | Gi evidence |
|---|---|---|
| `core/export-html/index.ts`, `ansi-to-html.ts`, `tool-renderer.ts`, `template.html`, `template.css`, `template.js` | `已有 / Go-native`; templates are generated by Go helpers and tested for XSS/whitespace/skill blocks | `export_html.go`, export tests |
| `core/export-html/vendor/highlight.min.js`, `vendor/marked.min.js` | `不适用`; Gi does not vendor JS runtime libraries for export | `syntax_highlight.go`, Go export renderer |
...
and edit diffs now render Pi-style line-numbered context windows instead of
raw replacement fragments.
