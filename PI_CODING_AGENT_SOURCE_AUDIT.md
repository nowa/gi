...
- Latest theme-core increment: custom theme validation now reserves `/` for
  ...
  calculation.
- Latest model-selector-runtime increment: `/model` consumes the canonical
  `ModelRuntime` snapshot immediately, then refreshes provider catalogs under a
  cancellable 15-second context. One selector mutex owns search, scope,
  selection, status, and close state; a completed refresh atomically replaces
  model definitions while preserving session-owned scoped thinking levels.
  Selection and cancellation stop the refresh before invoking host callbacks.
...
| `components/model-selector.ts`, `scoped-models-selector.ts`, `thinking-selector.ts`, `settings-selector.ts`, `config-selector.ts`, `oauth-selector.ts`, `theme-selector.ts`, `show-images-selector.ts` | `已有` for tested behavior including default `/scoped-models`, `/resources`, `/theme`, `/model` argument autocomplete with scoped-model filtering, model-selector cached-snapshot/background-refresh/close lifecycle, and Go-native `/login`/`/logout` provider selector TTY routing | `model_selector_component.go`, `oauth_selector.go`, `interactive_status.go`, `cli_interactive_tui.go`, settings/resource/auth selector tests |
...
Latest model-selector-runtime increment: that overlay now renders the immutable
cached `ModelRuntime` availability snapshot synchronously and refreshes catalogs
in the background. Refresh publication preserves current search/selection and
session-owned scoped thinking levels, reports timeout/provider/runtime status,
and requests a render only after a complete snapshot transition. Enter, Escape,
and explicit close cancel the refresh before the host callback runs, preventing
late catalog state from mutating a retired component.
