...
const (
	defaultClipboardCopyTimeout = 5 * time.Second
...
func CopyToClipboard(text string, options ClipboardCopyOptions) error {
	...
	commandOptions := ClipboardTextCommandOptions{Input: text, Timeout: defaultClipboardCopyTimeout}
	if !copied {
		switch platform {
		case "darwin":
		...
		default:
			...
			if !copied {
				...
				if isWayland && hasWaylandDisplay {
					if err := ops.ExecCommand("which wl-copy", ClipboardTextCommandOptions{Timeout: defaultClipboardCopyTimeout}); err == nil {
...
func runClipboardTextCommand(command string, options ClipboardTextCommandOptions) error {
	...
	if timeout <= 0 {
		timeout = defaultClipboardCopyTimeout
	}
