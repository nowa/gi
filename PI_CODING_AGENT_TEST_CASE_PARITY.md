...
| Pi line | Pi test case | Status | Gi coverage / next step |
|---:|---|---|---|
| 130 | should throw when prompt() called while streaming | 待实现 | 补 Gi coding-agent 本体 host/runtime 后添加 Go 对应测试 |
| 152 | should allow steer() while streaming | 待实现 | 补 Gi coding-agent 本体 host/runtime 后添加 Go 对应测试 |
| 168 | should allow followUp() while streaming | 待实现 | 补 Gi coding-agent 本体 host/runtime 后添加 Go 对应测试 |
| 184 | should queue extension-origin steering messages while streaming | 待实现 | 补 Gi coding-agent 本体 host/runtime 后添加 Go 对应测试 |
| 294 | should allow prompt() after previous completes | 待实现 | 补 Gi coding-agent 本体 host/runtime 后添加 Go 对应测试 |
| 339 | should wait for queued agent events before emitting tool_call | 待实现 | 补 Gi coding-agent 本体 host/runtime 后添加 Go 对应测试 |
| 484 | should persist message_end events in order with slow extension handlers | 待实现 | 补 Gi coding-agent 本体 host/runtime 后添加 Go 对应测试 |
...
| Pi line | Pi test case | Status | Gi coverage / next step |
|---:|---|---|---|
| 84 | executeBash resolves after the shell exits even if inherited stdio handles stay open | 待实现 | 补 Gi coding-agent 本体 host/runtime 后添加 Go 对应测试 |
| 109 | bash tool resolves after the shell exits even if inherited stdio handles stay open | 待实现 | 补 Gi coding-agent 本体 host/runtime 后添加 Go 对应测试 |
...
| Pi line | Pi test case | Status | Gi coverage / next step |
|---:|---|---|---|
| 10 | sends the clone RPC command | 需要协议 runtime | 按 protocol/spec 的 host actions、registry、capability、ViewTree 或 package resolver 落地 |
