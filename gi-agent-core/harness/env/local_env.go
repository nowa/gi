...
func ExecuteShellWithCapture(ctx context.Context, env *LocalExecutionEnv, command string, maxInlineBytes int) (CapturedShellResult, error) {
	...
	appendSanitized := func(chunk string) error {
		full.WriteString(strings.ReplaceAll(SanitizeBinaryOutput(chunk), "\r", ""))
		return nil
	}
	result, err := env.Exec(ctx, command, ExecOptions{
		OnStdout: appendSanitized,
		OnStderr: appendSanitized,
	})
	if err != nil {
	...
	output := full.String()
	if output == "" {
		output = strings.ReplaceAll(SanitizeBinaryOutput(result.Stdout+result.Stderr), "\r", "")
	}
