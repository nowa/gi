...
func TestIsContextOverflowErrorPatterns(t *testing.T) {
	tests := []struct {
		...
		{"openai parenthesized limit", "Input length (265330) exceeds model's maximum context length (262144).", true},
		{"openrouter maximum input", "Input length 265330 exceeds the maximum allowed input length of 262144 tokens.", true},
		{"ds4 configured context", "Prompt has 265,330 tokens, but the configured context size is 262,144 tokens.", true},
		{"dashscope input range", "Range of input length should be [1, 131072]", true},
