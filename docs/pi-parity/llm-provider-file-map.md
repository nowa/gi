...
| `providers/all.ts` | `getBuiltinModel`, `getBuiltinProviders`, `getBuiltinModelDataGeneratedAt`, `getBuiltinModels`, `builtinProviders`, `builtinModels`, `builtinImagesProviders`, `builtinImagesModels` | `builtin_catalog.go`, `builtin_providers.go` `GetBuiltinModel`, `GetBuiltinProviderIDs`, `GetBuiltinModelDataGeneratedAt`, `GetBuiltinModels`, `BuiltinProviders`, `BuiltinModels`; `pi_models_generated.go` manifest timestamp; existing image catalog/registry | partial | Text-provider assembly and immutable catalog reads are direct. The timestamp is generated from the validated release manifest, not maintained by hand. The image catalog and transport exist, but image providers do not yet have the equivalent instance-scoped `BuiltinImagesProviders` / `BuiltinImagesModels` assembly layer. |
...
| `providers/radius-config.ts` | `isRadiusGatewayModel`, `sanitizeRadiusGatewayConfig`, `normalizeRadiusGatewayUrl`, `getRadiusCredentialConfig`, `getRadiusModelsFromConfig`, `getRadiusModels`, `truncateHttpBody`, `loadRadiusGatewayConfig` | no complete equivalent | gap | Radius configuration validation, legacy credential import, and gateway catalog fetch remain to port. |
| `providers/radius.ts` | `radiusProvider` | skeletal `radius` entry in `builtin_providers.go` | gap | ID/auth metadata and an empty static catalog are declared, but dynamic refresh, `pi-messages`, and the OAuth loader remain open. |
...
- Provider-specific OAuth loaders advertised by the v0.82.0 factory layer are
...
- `providers/radius-config.ts`, the dynamic Radius refresh path, and the
  `api/pi-messages.ts` transport remain missing. The built-in `radius` entry is
  intentionally an empty, non-streamable catalog placeholder until those land.
...
- Text built-ins are instance-scoped; image catalog and OpenRouter image
  transport support still need an equivalent `ImagesModels` provider/auth
  assembly layer for `builtinImagesProviders` and `builtinImagesModels`.
