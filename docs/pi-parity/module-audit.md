...
| `packages/coding-agent/src/extensions` | `extension_discovery.go`, `extension_selector.go`, `protocol_extension_*`, and the Go-native bundled Llama files | consistent; generic extensions use the process protocol while Pi's trusted bundled llama.cpp extension is compiled into Gi and registered through the native model runtime. |
| `packages/coding-agent/src/extensions/llama` | `gi-coding-agent/internal/llama`, `llama_extension.go`, `llama_ui.go`, `cli_interactive_llama.go` | Go-native consistent; typed management/Hugging Face clients feed a mutex-owned provider catalog, the application workflow owns model lifecycle transitions, and a generation-checked TUI screen state owns interactive rendering. |
...
| `packages/coding-agent/src/modes/interactive/theme` | `interactive_theme_controller.go`, `terminal_theme.go`, `tui_theme.go`, `theme_export.go`, `theme-schema.json`, embedded asset/theme tests | Go-native consistent; one interactive controller owns terminal appearance, automatic synchronization, active palette, preview/restore, fallback, and listener disposal, while token/schema ownership remains separate and preserves Gi branding. |
...
| `gi-coding-agent/internal/browser` | `packages/coding-agent/src/utils/open-browser.ts` | `gi-coding-agent/cli_provider_auth_interaction.go` forwards OAuth launches while one utility owns shell-free platform command selection and asynchronous child reaping. |
...
| `gi-coding-agent/internal/jsonutil` | `packages/coding-agent/src/utils/json.ts` and `core/model-registry.ts` JSONC input | `gi-coding-agent/model_registry.go` keeps its package-local compatibility helper while a string-aware utility owns comment and trailing-comma removal. |
