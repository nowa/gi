...
| `packages/coding-agent/src/extensions/llama` | `gi-coding-agent/internal/llama` plus pending built-in command/TUI integration | partial; the typed management client, Hugging Face catalog, auth-aware native provider, synchronized lifecycle state, and offline contract tests are implemented. Built-in registration and interactive management remain open. |
