...
| `packages/ai/test/openrouter-oauth.test.ts` one-shot PKCE callback, permanent key, exchange failures, cancellation, and callback host | `TestOpenRouterOAuth*`, `TestBuiltinOpenRouterProviderUsesDefaultOAuth` | partial; the text-provider flow is covered, while shared text/image `ImagesModels` auth assembly remains open |
