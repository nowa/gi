...
| `packages/ai/test/models-runtime.test.ts` credential metadata, stored-credential ownership, request env/key overrides, mismatched-handler blocking, locked OAuth refresh, error preservation, and valid-token fast path | `TestInMemoryCredentialStore*`, `TestResolveProviderAuth*`, `TestAuthStorageCredentialStore*` | covered subset; provider collection, model refresh, stream assembly, and login/logout remain open |
| `packages/ai/test/oauth-auth.test.ts` shared OAuth credential derivation and refresh-storage boundary | `TestResolveProviderAuth*`, `TestAuthStorageCredentialStore*`, existing provider OAuth tests | partial; new Kimi, OpenRouter, xAI, and Radius flows remain open |
