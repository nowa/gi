...
| `packages/ai/test/oauth-auth.test.ts` shared OAuth credential derivation and refresh-storage boundary | `TestResolveProviderAuth*`, `TestAuthStorageCredentialStore*`, `TestRadiusOAuth*`, `TestXAIOAuth*`, existing provider OAuth tests | partial; Radius and xAI are complete, while the new Kimi and OpenRouter flows remain open |
