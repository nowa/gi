...
| `packages/ai/test/oauth-auth.test.ts` shared OAuth credential derivation and refresh-storage boundary | `TestResolveProviderAuth*`, `TestAuthStorageCredentialStore*`, `TestRadiusOAuth*`, existing provider OAuth tests | partial; Radius is complete, while the new Kimi, OpenRouter, and xAI flows remain open |
