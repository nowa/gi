...
| `packages/ai/test/oauth-auth.test.ts` shared OAuth credential derivation and refresh-storage boundary | `TestResolveProviderAuth*`, `TestAuthStorageCredentialStore*`, existing provider OAuth tests | partial; Radius request auth is covered, while its interactive login/refresh loader and new Kimi, OpenRouter, and xAI flows remain open |
